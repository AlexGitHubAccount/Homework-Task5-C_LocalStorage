...
let arr = [
    {name: "Немецкая", ingredients: {tomatoSauce, crimeanOnion, olives, mozzarella, oregano, parsley, pickledMushrooms}, image: "img/1.jpg", inCart: 0},
    {name: "Новинка", ingredients: {creamSauce, champignon, chickenLeg, pork,crimeanOnion, cherryTomato, mozzarella, parmesanCheese, oregano, basil}, image: "img/2.jpg", inCart: 0},
    {name: "Карбонара",  ingredients: {creamSauce, bacon, crimeanOnion, cherryTomato, mozzarella, parmesanCheese, oregano, basil}, image: "img/3.jpg", inCart: 0},
    {name: "Свит чили", ingredients: {sweetChiliSauce, pork, pineapples, bulgarianPepper,mozzarella, parmesanCheese, oregano, basil }, image: "img/4.jpg",inCart: 0},
    {name: "Беконная", ingredients: {tomatoSauce, mozzarella,tomato, bacon,parmesanCheese, oregano, parsley }, image: "img/5.jpg", inCart: 0},
    {name: "Куриная", ingredients: {sweetChiliSauce, chickenLeg, pineapples, bulgarianPepper, mozzarella, parmesanCheese, oregano, basil }, image: "img/6.jpg", inCart: 0},
    {name: "Крутая", ingredients: {tomatoSauce, mozzarella,tomato, chickenLeg, parmesanCheese, oregano, parsley }, image: "img/7.jpg", inCart: 0},
    {name: "Филона", ingredients: {tomatoSauce, mozzarella,tomato, bacon, chickenLeg, olives, parmesanCheese, oregano, parsley}, image: "img/8.jpg", inCart: 0},
    {name: "Ананасная", ingredients: {tomatoSauce, mozzarella, ham, olives, pineapples,  parmesanCheese, oregano }, image: "img/9.jpg", inCart: 0},
    {name: "Итальянская", ingredients: {tomatoSauce,tomato, chickenLeg, mozzarella, ham, bacon, champignon, bulgarianPepper,  parmesanCheese, oregano, parsley }, image: "img/10.jpg", inCart: 0},
    {name: "Маргарита", ingredients: {tomatoSauce, mozzarella, tomato, oregano}, image: "img/11.jpg", inCart: 0},
    {name: "Вкуснейшая", ingredients: {tomatoSauce, mozzarella,tomato, ham, parmesanCheese, oregano, parsley }, image: "img/12.jpg", inCart: 0},
    {name: "Убойная", ingredients: {tomatoSauce, mozzarella,tomato,bacon, balyk, parmesanCheese, oregano, parsley }, image: "img/13.jpg", inCart: 0},
    {name: "Китайская", ingredients: {creamSauce, champignon, chickenLeg, crimeanOnion, mozzarella, parmesanCheese, oregano }, image: "img/14.jpg", inCart: 0},
    {name: "Ветчинная", ingredients: {tomatoSauce, mozzarella,tomato, ham, crimeanOnion, oregano, parsley }, image: "img/15.jpg", inCart: 0},
];
...
function renderProducts(arr, productsView){//Build all structure for tiles and list
    if(productsView === 'tile'){//Build all structure for tiles inside block div.products-list
        for(let i = 0; i < arr.length; i++){
            ...
            var addToCartProduct = document.createElement('a');//Create tag a (button add to cart in HTML)
            addToCartProduct.href = '#';
            addToCartProduct.classList.add('product-card__add-cart');//Create tag a (button add to cart in HTML)
            addToCartProduct.innerText = 'Add to cart';
...
if(productsView === 'tile'){
    allCardsTiles.addEventListener( 'click', function(e) {
        if(e.target.closest('.product-card') && e.target.closest('.product-card__add-ingredients') === null && e.target.closest('.product-card__add-cart') === null){
            ...
            for(let i = 0; i< arr.length; i++){
                if(e.target.parentElement.querySelector('.product-card__name').innerText === arr[i].name){
                    cartNumbers(arr[i]);
                }
            }
        }
...
// ------------------------------
//Third part of third homework
// Create an object in the localstorage and include amounts of clicks by buttons in the red circle (our cart in the html)
let cartAmount = document.querySelector('.circleForCart');
...
function cartNumbers(product){
    ...
    if(productNumbers){
        ...
        document.querySelector('.circleForCart').textContent = productNumbers + 1;
    } else {
        localStorage.setItem('cartNumbers', 1);
        document.querySelector('.circleForCart').textContent = 1;
        cartAmount.classList.remove('hidden');
    }
    setItems(product);
}
